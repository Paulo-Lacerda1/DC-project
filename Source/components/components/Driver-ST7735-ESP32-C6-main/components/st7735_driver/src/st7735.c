...
esp_err_t st7735_init(const st7735_config_t *cfg) {
    ...
    if (!cfg->reuse_spi_bus) {
        ...
        // Quando o SD já configurou o barramento, basta adicionar o dispositivo sem reiniciar
        ESP_LOGI(TAG, "A reutilizar barramento SPI já inicializado (host %d).", cfg->host_id);
    }
